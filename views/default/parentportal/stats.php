...
$child = elgg_extract('child', $vars);

$blog_label = elgg_echo('parentportal:stats:blog');
$blog_count = elgg_get_entities(array('owner_guid' => $child->guid, 'type' => 'object', 'subtype' => 'blog', 'count' => true));
$blog_count = $blog_count ? $blog_count : 0;

$photo_label = elgg_echo('parentportal:stats:photo');
$photo_count = elgg_get_entities(array('owner_guid' => $child->guid, 'type' => 'object', 'subtype' => 'image', 'count' => true));
$photo_count = $photo_count ? $photo_count : 0;

$bookmark_label = elgg_echo('parentportal:stats:bookmark');
$bookmark_count = elgg_get_entities(array('owner_guid' => $child->guid, 'type' => 'object', 'subtype' => 'bookmarks', 'count' => true));
$bookmark_count = $bookmark_count ? $bookmark_count : 0;

if (elgg_is_active_plugin('todo')) {
	$todo_label = elgg_echo('parentportal:stats:todo');
	
	$todo_count = count_complete_todos($child->guid);
